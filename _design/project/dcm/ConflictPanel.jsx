// ConflictPanel.jsx — Right sidebar: lists all active conflicts
const { useState: cpSt } = React;

function ConflictItem({ conflict, tickets, project, onOpenTicket }) {
  const [open, setOpen] = cpSt(false);
    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <Badge type={isHard ? 'hard' : 'soft'} size="xs">
            {isHard ? '⚡ Hard' : '◎ Soft'}
          </Badge>
      ...
      {open && (
        <div style={{
          ...
          <p style={{ margin: 0, fontSize: 10, color: C.textMut, lineHeight: 1.55 }}>
            {isHard
              ? `Both tickets are deploying to ${env1?.name}. Any overlapping module change can cause direct conflicts in production.`
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            color: C.textMut, fontSize: 16, padding: '0 2px', lineHeight: 1,
            fontFamily: 'Inter, sans-serif',
          }}
        >×</button>
      </div>
      ...
      {pConf.length > 0 && (
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: hardC.length > 0 ? C.hard : C.textMut, lineHeight: 1 }}>
              {hardC.length}
            </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: softC.length > 0 ? C.soft : C.textMut, lineHeight: 1 }}>
              {softC.length}
            </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: C.textSec, lineHeight: 1 }}>
              {pConf.length}
            </span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px', display: 'flex', flexDirection: 'column', gap: 7 }}>
        {pConf.length === 0 ? (
          <div style={{
            ...
            textAlign: 'center', gap: 8, padding: '32px 16px',
            color: C.textMut,
          }}>
            <div style={{
          ...
          <>
            ...
            {hardC.map(c => (
              <ConflictItem
                ...
                tickets={tickets}
                project={project}
                onOpenTicket={id => dispatch({ type: 'OPEN_TICKET', id, mode: 'view' })}
              />
            ))}
            {softC.length > 0 && (
...
Object.assign(window, { ConflictPanel });
