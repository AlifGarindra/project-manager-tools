// Dashboard.jsx — Project selection landing
const { useState: dSt } = React;
...
function ProjectCard({ project, allTickets, conflicts, onSelect }) {
  const [hov, setHov] = dSt(false);
  ...
  const hasConflict = hardC > 0;

  // Upcoming tickets (next 14 days)
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${hasConflict ? 'rgba(239,68,68,0.35)' : hov ? C.borderEl : C.border}`,
        borderRadius: 8,
        padding: '18px 20px',
        cursor: 'pointer',
  ...
  return (
    ...
    >
      <div style={{
        ...
        fontSize: 18, color: hov ? C.textSec : C.textMut,
        transition: 'all 0.12s',
      }}>+</div>
...
function DashboardView() {
  ...
  const [showNew, setShowNew] = dSt(false);
  const [newName, setNewName] = dSt('');
  const [newDesc, setNewDesc] = dSt('');
  ...
  const createProject = () => {
    ...
    dispatch({ type: 'SAVE_PROJECT', project: p });
    setShowNew(false);
    setNewName('');
    setNewDesc('');
  };
        ...
        <div style={{ marginBottom: 32, display: 'flex', alignItems: 'flex-end', gap: 16 }}>
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: 0, fontSize: 12, color: C.textMut }}>
              ...
              {state.tickets.length} total tickets ·{' '}
              {totalConflicts > 0
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14 }}>
          {state.projects.map(p => (
            <ProjectCard
              ...
              conflicts={conflicts}
              onSelect={id => dispatch({ type: 'SET_PROJECT', id })}
            />
          ))}
          ...
          >
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ display: 'flex', gap: 7, justifyContent: 'flex-end' }}>
                ...
                <Btn variant="primary" size="sm" onClick={createProject}>Create Project</Btn>
              </div>
            </div>
          </div>
        )}
